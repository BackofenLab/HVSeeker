...
def encode_string(maxLen=None, x=[], y=[], y_encoder=None, repeat=True, use_spacer=False, online_Xtrain_set=False,
    ...
    if len(x) > 0:
        ...
        if online_Xtrain_set:
            X_train_categorial = []
            for seq in out:
                X_train_categorial.append(np.array(to_categorical(seq, num_classes=len(a)), dtype=bool))
...
def shrink_timesteps(X, Y, input_subSeqlength=0):
    ...
    if len(X.shape) == 3:
        ...
        bigarray = []
        for sample in X:
            sample = np.array(sample[0:newSeqlength], dtype=bool)
        ...
        sample = np.array(X[0:newSeqlength], dtype=bool)
        subarray = sample.reshape((int(seqlength / subSeqlength), subSeqlength, features))
        X = np.array(subarray)
